...
describe("FHEContract", function () {
  ...
  describe("Deployment", function () {
    it("Should have the right value on deploy", async function () {
      ...
      await hre.fhe.mocks.expectPlaintext(val, 0n);
    });
    ...
    it("Should allow setting and reading a value", async function () {
      ...
      const encryptedInputResult = await fhe.encrypt([Encryptable.uint32(targetValue)] as const);
      const [inputValue] = await hre.fhe.expectResultSuccess(encryptedInputResult);
      ...
      // Unseal the value
      const unsealResult = await fhe.unseal(valCtHash, FheTypes.Uint32);
      const valUnsealed = await hre.fhe.expectResultSuccess(unsealResult);
    ...
    it("Should allow performing operations on the value", async function () {
      ...
      // Encrypt target value, and pass it to the contract
      const encryptedInputResult = await fhe.encrypt([
        Encryptable.uint32(targetValue),
        Encryptable.uint32(operandValue),
      ] as const);
      const [inputValue, operandInputValue] = await hre.fhe.expectResultSuccess(encryptedInputResult);
      await fheContract.setVal(inputValue);

      // Use the mocks to peek at the value
      let valCtHash = await fheContract.val();
      await hre.fhe.mocks.expectPlaintext(valCtHash, targetValue);

      // Perform addition on the value
      await fheContract.valOp(0, operandInputValue);
      valCtHash = await fheContract.val();
      await hre.fhe.mocks.expectPlaintext(valCtHash, targetValue + 2n);
